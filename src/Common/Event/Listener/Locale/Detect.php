    ...
    public function execute(): void
    {
        ...
        if ($oLocale::ENABLE_SNIFF_URL) {
            ...
            if (array_key_exists('PATH_INFO', $_SERVER)) {
                $_SERVER['PATH_INFO'] = '/' . preg_replace($oLocale->getUrlRegex(), '$2', ltrim($_SERVER['PATH_INFO'], '/'));
            }

            if (array_key_exists('REQUEST_URI', $_SERVER)) {
                $_SERVER['REQUEST_URI'] = '/' . preg_replace($oLocale->getUrlRegex(), '$2', ltrim($_SERVER['REQUEST_URI'], '/'));
            }
        }
    }
}
