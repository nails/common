...
class NAILS_Fb extends NAILS_Auth_Controller
{
	...
	protected function _connect_success()
	{
		...
		$_user = $this->user_model->get_by_fbid( $_me['id'] );
		...
		if ( ! $_user ) :
			...
			$_user = $this->user_model->get_by_email( $_me['email'] );
	...
	protected function _login_user( $user )
	{
		...
		if ( $this->config->item( 'auth_two_factor_enable' ) ) :
			...
			if ( ! $_token ) :
				...
				show_fatal_error( 'Failed to generate two-factor auth token', 'A user tried to login with Facebook and the system failed to generate a two-factor auth token.' );
			...
			redirect( 'auth/security_questions/' . $user->id . '/' . $_token['salt'] . '/' . $_token['token'] . '/facebook' . $_query );
			...
			$_return = $this->_return_to ? $this->_return_to : $user->group_homepage;
	...
	protected function _create_user( $me )
	{
		...
		$_data['email_is_verified']	= TRUE;	//	Trust the email from Facebook
