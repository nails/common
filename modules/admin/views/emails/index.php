...
<p>Below are all the emails which are pending send-out by the system, view the details of each item by clicking the 'View' button in the 'Variables' column.</p>
<p><em>Note: these are individual mailshots which are to be sent, depending on users preferences these may be merged into a single email (a daily digest for example).</em></p>

<section class="filter-box">
	<p style="margin:0;padding:0;">
		<form method="get" action="<?=site_url( 'admin/emails/index' )?>" class="form" style="margin:0;padding:0;">
		<label>Search:</label>
...
<table>
	...
	<tbody>
		...
		<?php if ( count( $queued_mail ) ) : ?>
		<?php foreach ( $queued_mail AS $e ) : ?>
			...
			<tr>
				...
				<td><?=$e->name?></td>
				<td><?=nice_time( (int) $e->time_queued )?></td>
				<td class="vars">
				...
				<td class="options">
					...
					<?=anchor( 'admin/emails/index/delete/' . $e->id, 'Delete', 'class="delete a-button a-button-small"' )?>
