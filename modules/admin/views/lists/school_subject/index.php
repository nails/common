...
<div class="left" style="margin-left:10px;margin-right:10px;width:400px;">
	...
	<div class="box">
		...
		<h2>Add New Subject</h2>
			...
			<?=form_open_multipart( 'admin/lists/school_subject' )?>
	...
	<?php
		...
		//	For each school_subject result, pull the stats
		foreach ( $school_subjects->result() AS $i ) :
			$top_subjects[$i->name] 		= $i->u_count;
		endforeach;
...
<div class="left box" style="margin-left:10px;margin-right:10px;width:auto;">
	...
	<table id="subjects-list" style="padding:0 10px 10px;">
		...
		<tbody>
			...
			<?php $temp = array(); ?>
			<?php foreach ( $school_subjects->result() AS $i ) : ?>
			...
			<tr>
				...
				<td width="88">
					...
					<?=anchor( 'admin/lists/school_subject_edit/' . $i->id, 'Edit', 'class="a-button a-button-small"' )?>
					<?php if ( $i->u_count == 0 ) : ?>
						<?=anchor( 'admin/lists/school_subject_delete/' . $i->id, 'Delete', 'class="a-button a-button-small confirm-delete"' )?>
					<?php endif; ?>				
				</td>
