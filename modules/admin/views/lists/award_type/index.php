...
$(function () 
{
	...
	$( 'input[name=type]' ).keyup( function() {
		...
		$( '#award_type-list' ).find( '.searchable' ).each( function() {
		...
		//	How many results
		$( '#award_type-list .searchable' ).each( function() {
...
<h1>Manage Award &amp; Scholarship Types</h1>
...
<div class="left" style="margin-left:10px;margin-right:10px;width:400px;">
	...
	<div class="box">
		...
		<h2>Add New Type</h2>
			...
			<?=form_open_multipart( 'admin/lists/award_type' )?>
	...
	<?php
		...
		//	Set up arrays
		$top_award_type 	= array();
		
		//	For each award_type result, pull the stats
		foreach ( $award_types->result() AS $i ) :
			$top_award_type[$i->type] = $i->u_count;
		endforeach;
		
		//	Sort by value, high to low
		arsort($top_award_type);
		$top_award_type = array_slice($top_award_type, 0, 5, TRUE);
	...
	<div class="box" style="margin-top:20px;padding:">
		...
		<?php
			...
			echo '<ul>';
			foreach ( $top_award_type AS $tsi => $val ) :
...
<div class="left box" style="margin-left:10px;margin-right:10px;width:auto;">
	...
	<table id="award_type-list" style="padding:0 10px 10px;">
			
		<tbody>
		
			<?php $temp = array(); ?>
			<?php foreach ( $award_types->result() AS $i ) : ?>
			...
			<tr>
				...
				<td width="88">
					...
					<a href="<?=site_url( 'admin/lists/award_type_edit/' . $i->id ) ?>" class="a-button a-button-small">Edit</a>
					<?php if ( $i->u_count == 0 ) : ?>
						<a href="<?=site_url( 'admin/lists/award_type_delete/' . $i->id )?>" class="a-button a-button-small confirm-delete">Delete</a>
					<?php endif; ?>						
				</td>
