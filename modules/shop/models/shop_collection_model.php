...
class NAILS_Shop_collection_model extends NAILS_Model
{
	...
	{
		$this->db->select( 'c.id,c.slug,c.label,c.description,c.seo_description,c.seo_keywords,c.is_active,c.created,c.modified' );

		if ( $include_count ) :

			$this->db->select( '(SELECT COUNT(*) FROM ' . NAILS_DB_PREFIX .  'shop_product_collection WHERE collection_id = c.id) product_count' );

		endif;

		$this->db->order_by( 'c.is_active', 'DESC' );
		$this->db->order_by( 'c.label' );
		$_result = $this->db->get( $this->_table . ' c' )->result();
	...
	public function create( $data )
	{
		...
		if ( isset( $data->is_active ) ) :
			...
			$_data->is_active = (bool) $data->is_active;
...
/* Location: ./application/models/shop_collection_model.php */