...
class NAILS_Shop_range_model extends NAILS_Model
{
	...
	{
		$this->db->select( 'r.id,r.slug,r.label,r.description,r.seo_description,r.seo_keywords,r.is_active,r.created,r.modified' );

		if ( $include_count ) :

			$this->db->select( '(SELECT COUNT(*) FROM ' . NAILS_DB_PREFIX .  'shop_product_range WHERE range_id = r.id) product_count' );

		endif;

		$this->db->order_by( 'r.is_active', 'DESC' );
		$this->db->order_by( 'r.label' );
		$_result = $this->db->get( $this->_table . ' r' )->result();
