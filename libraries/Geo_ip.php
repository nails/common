 ...
 * - Corrected includes for use in Nails.
 * - Party/Fiesta
 *
 */
...
class Geo_ip{
	...
	function __construct()
	{
		...
		$this->CI->config->load( 'geoip', TRUE, TRUE );

		// --------------------------------------------------------------------------

		//	Check the database exists, if it doesn't fail gracefully
		$_db = $this->CI->config->item( 'database', 'geoip' );

		if ( $_db ) :

			define( 'GEOIP_FILEDATA', $_db );

		else :

			//	Use default
			define( 'GEOIP_FILEDATA', NAILS_PATH . 'libraries/_resources/geoip/GeoLiteCity.dat' );

		endif;

		// --------------------------------------------------------------------------

		if ( ! file_exists( GEOIP_FILEDATA ) ) :

			$this->_db_exists = FALSE;
			log_message( 'debug', 'GeoIP: Database not found [' . GEOIP_FILEDATA . ']' );

			if ( ENVIRONMENT != 'production' ) :

				trigger_error( 'GeoIP: Database not found [' . GEOIP_FILEDATA . ']. Please <a href="http://docs.nailsapp.co.uk/libraries/geoip#installation">refer to the documentation</a> for installation instructions.' );
